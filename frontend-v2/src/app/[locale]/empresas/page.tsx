...
export default async function V2Empresas({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  const { locale } = await params
  const tr = await getTranslations({ locale, namespace: 'v2.empresas' })
  const [COMPANIES, WELLS] = await Promise.all([loadCompanies(), loadWells()])
  ...
  return (
    <>
      ...
      <Seccion n="02" titulo={tr('s02t')} desc={tr('s02d')}>
        <ListaEmpresas
          filas={filas}
          grupos={grupos}
      ...
      >
        <Card>
          ...
          {TRAMOS.map((tm, i) => {
            const g = COMPANIES.filter(tm.test)
            ...
            return (
              ...
              >
                <span className="min-w-[140px] flex-[1_1_150px]">
                  <span className="s-cuerpo block font-medium">{tr(tm.tramo)}</span>

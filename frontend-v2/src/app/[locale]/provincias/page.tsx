...
import { loadCompanies } from '@/lib/data/companies'
import { loadNationalSeries } from '@/lib/data/production'
...
/* PROVINCIAS — pocas filas, así que acá SÍ va la barra en cada una: con ocho
   ...
   DATOS: loadProvinces (API /v2/provinces + export-summary, contenido
   editorial mergeado del fixture) + serie nacional del loader. */

export const revalidate = 3600

export default async function V2Provincias({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  const [PROVINCES, COMPANIES, NATIONAL_SERIES] = await Promise.all([
    loadProvinces(),
    loadCompanies(),
    loadNationalSeries(),
  ])
  const SOLO_PROVINCIAS = PROVINCES.filter((p) => p.esProvincia !== false)
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'v2.provincias' })
  ...
  return (
    <>
          ...
          items={[
            {
              ...
              apoyo: t('apoCuencas', { n: cuencas.length }),
      ...
      >
        <Card>
          <CardHead titulo={t('s02t')} nota={`${formatInteger(totalPozos)} pozos`} />
          {porPozos.map((p, i) => (
            <FilaProvincia
              key={p.slug}
              p={p}
      ...
      >
        <Card>
          ...
          <CardHead titulo={t('s03t')} nota={`${formatCompactAR(totalExpo)} MUSD · ${t('apoAnio')}`} />
      ...
      >
        <Card>
          ...
          <CardHead titulo={t('s04t')} nota={`${formatInteger(pozosEnCuencas)} pozos`} />
